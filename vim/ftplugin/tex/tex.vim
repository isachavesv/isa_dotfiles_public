...
call IMAP('{{', "\\{<++>\\}<++>", 'tex')
call IMAP('{'.mapleader, "\\left\\{<++>\\right\\}<++>", 'tex')
call IMAP('[[', "\\left[<++>\\right]<++>", 'tex')
call IMAP(mapleader.'iid', "\\overset\\{\sim\\}\\{iid\\}<++>", 'tex')

"color in slides
call IMAP('RRED', '{\color{red}<++>}','tex')
"
...
call IMAP('EEQ', "\\begin{equation}\<CR><++>\<CR>%\\label{<++>}\<CR>\\notag\<CR>\\end{equation}\<CR><++>", 'tex')
"Good shortcuts
...
let g:Tex_Com_only = "\\only<<+number+>>{\<CR><++>\<CR>}"
let g:Tex_Com_alert = "\\alert<<+number+>>{\<CR><++>\<CR>}"

let g:Tex_Com_frac = "\\frac{\<CR><++>\<CR>}\<CR>{\<CR><++>\<CR>}\<CR><++>"
 
"quick bracing
let g:Tex_Com_underbrace = "\\underbrace{<++>}_{<++>}<++>"

let g:Tex_Com_overbrace = "\\overbrace{<++>}_{<++>}<++>"
